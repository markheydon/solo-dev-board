...
/// <summary>Backlog Review grouping panel rendered inside <see cref="PlanningShell"/>.</summary>
public partial class PlanningBacklogPanel : ComponentBase
{
    internal const string AllTypesFilter = PlanningItemKindFormatting.AllTypesFilter;
    internal const string IssueTypeFilter = PlanningItemKindFormatting.IssueTypeFilter;
    internal const string PullRequestTypeFilter = PlanningItemKindFormatting.PullRequestTypeFilter;
    ...
    [Inject]
    public ILogger<PlanningBacklogPanel> Logger { get; set; } = default!;
    ...
    {
        if (!PlanningItemKindFormatting.MatchesTypeFilter(item.ItemType, selectedTypeFilter))
        {
            return false;
        }
    ...
    private static string FormatItemKindChip(PlanningWorkItemTypeDto itemType)
        => PlanningItemKindFormatting.FormatLabel(itemType);

    private static Color FormatItemKindChipColor(PlanningWorkItemTypeDto itemType)
        => PlanningItemKindFormatting.FormatChipColor(itemType);
    ...
    private static string? FormatPartialFailureWarning(IReadOnlyList<PlanningRepositoryCatalogueFailureDto> failures)
    {
        if (failures.Count == 0)
        {
            return null;
