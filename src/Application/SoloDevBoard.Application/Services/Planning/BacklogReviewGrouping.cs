...
public static class BacklogReviewGrouping
{
        ...
        DateTimeOffset referenceTimeUtc)
    {
        ...
        foreach (var boardItem in boardItems)
        {
            ...
            boardStatusByKey.TryAdd(PlanningWorkItemJoinKey.For(boardItem), statusName);
        }
        ...
        {
            boardStatusByKey.TryGetValue(PlanningWorkItemJoinKey.For(workItem), out var boardStatusName);
    ...
    public static bool IsUrgent(PlanningWorkItemDto item)
    {
        ArgumentNullException.ThrowIfNull(item);
        return PlanningLabelHelpers.IsUrgent(item.Labels);
    }
    ...
    public static bool IsAwaitingTriage(PlanningWorkItemDto item)
    {
        ArgumentNullException.ThrowIfNull(item);
        return PlanningLabelHelpers.IsAwaitingTriage(item.Labels);
    }
    ...
    public static bool IsReadyToStart(PlanningWorkItemDto item, string? boardStatusName)
    {
        ArgumentNullException.ThrowIfNull(item);
        ...
        if (!PlanningLabelHelpers.IsUnblocked(item.Labels))
        {
            return false;
        }
    ...
    public static bool IsBlockedOrDeferred(PlanningWorkItemDto item, string? boardStatusName)
    {
        ArgumentNullException.ThrowIfNull(item);
        return PlanningLabelHelpers.IsBlockedOrDeferred(item.Labels) || IsParkedBoardStatus(boardStatusName);
    }
    ...
    public static bool IsNeglected(
        PlanningRepositorySummaryDto summary,
        int neglectDays,
        DateTimeOffset referenceTimeUtc)
    {
        ...
        out BacklogEpicNearCompleteItemDto epicNearComplete)
    {
        ...
        if (!PlanningLabelHelpers.IsEpicNearComplete(item.Labels, item.SubIssueTotal, item.SubIssueCompleted))
        {
            return false;
        }

        var typeLabel = PlanningLabelHelpers.ParseTypeLabel(item.Labels)!;
    ...
    private static bool HasOpenEpicsWithoutSubIssueCounts(IReadOnlyList<PlanningWorkItemDto> workItems)
    {
        var openEpicsOrFeatures = workItems
            .Where(static item => item.ItemType == PlanningWorkItemTypeDto.Issue)
            .Select(static item => (Item: item, TypeLabel: PlanningLabelHelpers.ParseTypeLabel(item.Labels)))
            .Where(static pair => pair.TypeLabel is PlanningLabelHelpers.EpicTypeLabel or PlanningLabelHelpers.FeatureTypeLabel)
            .ToArray();
    ...
    private static BacklogReviewItemDto Map(PlanningWorkItemDto item, string? boardStatusName)
        => new(
            item.ItemType,
            item.RepositoryFullName,
            item.Number,
    ...
    private static IReadOnlyList<BacklogReviewItemDto> Sort(IReadOnlyList<BacklogReviewItemDto> items)
        => items
            .OrderBy(static item => PlanningPriorityRanker.GetRank(item.PriorityLabel))
