...
public sealed class PlanningWorkItemCatalogueService(
    IGitHubService gitHubService,
    IPlanningSettingsService pmSettingsService,
    ILogger<PlanningWorkItemCatalogueService> logger) : IPlanningWorkItemCatalogueService
{
    ...
    /// </remarks>
    public async Task<PlanningWorkItemCatalogueResultDto> GetCatalogueAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new PlanningWorkItemCatalogueResultDto(items, failures, summaries);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        PlanningRepositoryCatalogueFailureDto? failure = null;
        if (issueResult.ErrorMessage is not null || pullRequestResult.ErrorMessage is not null)
        {
            failure = new PlanningRepositoryCatalogueFailureDto(
                repositoryFullName,
        ...
        var items = new List<PlanningWorkItemDto>(
    ...
    private static PlanningWorkItemDto MapIssue(
        Issue issue,
        string repositoryFullName,
    ...
    private static PlanningWorkItemDto MapPullRequest(
    ...
    private static IReadOnlyList<PlanningRepositorySummaryDto> BuildRepositorySummaries(
        ...
        IReadOnlyList<PlanningWorkItemDto> items)
    {
        ...
        return includedRepositories
            ...
            .Select(repository =>
            {
                ...
                return new PlanningRepositorySummaryDto(
                    repository.FullName,
                    repositoryItems.Count(static item => item.ItemType == PlanningWorkItemTypeDto.Issue),
                    repositoryItems.Count(static item => item.ItemType == PlanningWorkItemTypeDto.PullRequest),
                    lastActivity,
                    IsIncluded: true);
            })
            .ToArray();
    ...
    private static bool IsEpicOrFeature(IReadOnlyList<string> labels)
    {
        ...
        return typeLabel is PlanningLabelHelpers.EpicTypeLabel or PlanningLabelHelpers.FeatureTypeLabel;
    }
    ...
    private sealed record RepositoryCatalogueLoadResult(
        IReadOnlyList<PlanningWorkItemDto> Items,
        PlanningRepositoryCatalogueFailureDto? Failure);
}
