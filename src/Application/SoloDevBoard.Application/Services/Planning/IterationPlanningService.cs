...
public sealed class IterationPlanningService : IIterationPlanningService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var joinKey = PlanningWorkItemJoinKey.For(itemType == PlanningWorkItemTypeDto.PullRequest, repositoryFullName, number);
        var existingItem = catalogue.Items.FirstOrDefault(item =>
            PlanningWorkItemJoinKey.For(item).Equals(joinKey, StringComparison.OrdinalIgnoreCase));
