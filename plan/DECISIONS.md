...
**Summary:** SoloDevBoard uses **MudBlazor snackbars** for brief, non-blocking operation outcomes (success, warning, error, validation) and **inline `MudAlert` / status regions** for persistent page-local state (load errors with retry, empty states, setup guidance, preview panels, multi-item batch results). Do not show the same message in both channels. Operation-complete feedback on long pages must not require scrolling on a typical viewport; prefer snackbars for those outcomes. Global defaults remain in `Program.cs` (bottom-right, five seconds, outlined). Reject bottom-of-page inline alerts for transient CRUD or apply summaries when a snackbar suffices.
