...
**Summary:** Use MudBlazor for all Blazor UI. Prefer MudBlazor layout primitives and utility classes before custom CSS. Reject Fluent UI Blazor and raw HTML controls where a MudBlazor equivalent exists (supersedes legacy ADR-0009). Component choice and page chrome follow the `mudblazor` skill pinned to the NuGet version in `Directory.Packages.props` (9.9.0 as of DEC-039).
...
**Summary:** After the public `v1.0.0` tag, do not ship a `v0.x` release. The original split (deferred slices on **v1.1.0**, Cross-Repo Planning on **v1.2.0**) was replaced by a single active milestone model in DEC-027.
...
**Summary:** After `v1.0.0`, maintain **one open GitHub milestone** at a time. **`v1.3 - Usable solo workflow across repositories`** is the current open milestone (tag **`v1.3.0`** when shipped). Locked-in start: Label Manager remap extras ([#491](https://github.com/markheydon/solo-dev-board/issues/491)); further issues will be added from dogfood. **`v1.2 - Planning polish, Reload & Templates`** is closed (tag [`v1.2.0`](https://github.com/markheydon/solo-dev-board/releases/tag/v1.2.0), 2026-09-07). **`v1.1 - Cross-Repo Planning & Refinement`** is closed (91 closed, 0 open; tag [`v1.1.0`](https://github.com/markheydon/solo-dev-board/releases/tag/v1.1.0), 2026-08-31). Delivered `v1.2` scope: [#403](https://github.com/markheydon/solo-dev-board/issues/403) (board-scoped Daily Focus), Reload epic [#447](https://github.com/markheydon/solo-dev-board/issues/447) with [#449](https://github.com/markheydon/solo-dev-board/issues/449)–[#451](https://github.com/markheydon/solo-dev-board/issues/451), HttpClient split [#453](https://github.com/markheydon/solo-dev-board/issues/453), custom template repositories [#292](https://github.com/markheydon/solo-dev-board/issues/292), and Triage action-surface usability [#492](https://github.com/markheydon/solo-dev-board/issues/492). Window-focus refetch [#450](https://github.com/markheydon/solo-dev-board/issues/450) stays unmilestoned and ice-boxed. Do not recreate a separate milestone whose only purpose is Cross-Repo Planning — that increment shipped under `v1.1` (the brief unused v1.2.0 planning-only milestone was deleted on 2026-08-18). Platform-blocked work ([#293](https://github.com/markheydon/solo-dev-board/issues/293), [#391](https://github.com/markheydon/solo-dev-board/issues/391), [#498](https://github.com/markheydon/solo-dev-board/issues/498)) and ice-box catalogue work stay unmilestoned unless explicitly promoted. **Epic → Feature → Story** applies when each level names real product structure: Epics are shippable themes spanning multiple features (not milestone buckets); Features group related stories/enablers ([#272](https://github.com/markheydon/solo-dev-board/issues/272) is a catch-up exception — classified as a feature without a parent epic); Stories are delivery units. Use GitHub sub-issues for parent links. During catch-up, milestone stories may sit without Feature parents when they complete shipped features ([#40](https://github.com/markheydon/solo-dev-board/issues/40), [#88](https://github.com/markheydon/solo-dev-board/issues/88), etc.). Implementation **phases** in `IMPLEMENTATION_PLAN.md` are historical sequencing for the v1.0 release only; the Project board **Phase** field is legacy for closed pre-1.0 milestones. Reject bucket epics that duplicate milestones, fabricated Features, and multiple concurrent open milestones.

**Milestone naming:** GitHub milestones use `vX.Y - Descriptive name` (ASCII hyphen with spaces; no patch segment). Git release tags keep SemVer `vX.Y.Z` (MinVer). See [`plan/PROJECT_MANAGEMENT.md`](PROJECT_MANAGEMENT.md#milestones).
...
### DEC-029: Cross-Repo Planning board selection and local settings
...
**Summary:** User-facing names are **Actions Templates** (GitHub Actions workflow template library at `/actions-templates`) and **Planning** (cross-repo planning hub at `/planning`, Iteration tab at `/planning/iteration`). Routes, C# namespaces (`Features.ActionsTemplates`, `Planning`), and `data-testid` prefixes follow the product names. Legacy URL aliases (`/workflows`, `/pm-workflow`, `/audit`) are not supported. Domain types `WorkflowFile` and `WorkflowRun` remain for GitHub workflow file/run data (Audit KPI and template apply). This repository's GitHub `area/*` labels use `area/actions-templates` and `area/planning`; `area/dashboard` is Audit Dashboard only. Label Manager's built-in recommended catalogue omits `area/*` ([#446](https://github.com/markheydon/solo-dev-board/issues/446)). Reject reverting nav labels to **Workflows** / **PM Workflow** or mapping Planning work to `area/dashboard`.
...
**Summary:** Label Manager's built-in recommended catalogue is the **portable** workflow set (`type/`, `priority/`, `status/`, `size/`). `area/*` names describe SoloDevBoard's own feature map and must not be created on other repositories by Recommended taxonomy or Synchronise. When **Remove labels outside taxonomy** is on, a nested **Keep `area/*` labels** control (default on) excludes those names from extra deletes; unchecking it treats them as genuine extras. Hard-code the `area/` prefix for `v1.1`; do not add app-settings ignore lists yet. Bulk delete on the Labels tab (#444) is a separate confirmation loop and does not add a protected allow-list. Reject exporting this repository's area map as product-default labels.

---

### DEC-035: Transient feedback via snackbar

**Status:** Active  
**Date:** 2026-08-25  
**Related:** [#465](https://github.com/markheydon/solo-dev-board/issues/465), [`.agents/skills/mudblazor/references/FEEDBACK-OVERLAYS.md`](../.agents/skills/mudblazor/references/FEEDBACK-OVERLAYS.md)  
**Summary:** SoloDevBoard uses **MudBlazor snackbars** for brief, non-blocking operation outcomes (success, warning, error, validation) and **inline `MudAlert` / status regions** for persistent page-local state (load errors with retry, empty states, setup guidance, preview panels, multi-item batch results). Do not show the same message in both channels. Do not use snackbars for in-progress feedback when inline progress indicators already communicate work in flight. Error snackbars use a six-second visibility duration; other severities use five seconds. Operation-complete feedback on long pages must not require scrolling on a typical viewport; prefer snackbars for those outcomes. Global defaults remain in `Program.cs` (bottom-right, outlined). Reject bottom-of-page inline alerts for transient CRUD or apply summaries when a snackbar suffices.
...
**Summary:** Shipped pages must share one visual language: page header plus purpose line, toolbar for commands, `MudPaper`/`MudCard` sections at elevation 1, `MudSkeleton` for content-shaped loading, `MudAlert` for empty and error, snackbars for transient outcomes (DEC-035). Agents choose components from MudBlazor **9.9.0** via the `mudblazor` skill; do not use `MudChat` (not in 9.9.0), FABs, carousels, or charts on operational pages. Custom CSS remains exceptional. Product logo and brand assets stay a separate ice-box feature ([#397](https://github.com/markheydon/solo-dev-board/issues/397)).
