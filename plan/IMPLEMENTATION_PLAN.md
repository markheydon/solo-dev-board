...
- **`v1.2 - Planning polish, Reload & Templates`** is ready to tag (30 closed, 0 open on the GitHub milestone; release tag `v1.2.0` after staging smoke test): Daily Focus board-scope [#403](https://github.com/markheydon/solo-dev-board/issues/403), Reload [#447](https://github.com/markheydon/solo-dev-board/issues/447)/[#449](https://github.com/markheydon/solo-dev-board/issues/449)/[#451](https://github.com/markheydon/solo-dev-board/issues/451), HttpClient split [#453](https://github.com/markheydon/solo-dev-board/issues/453), custom templates [#292](https://github.com/markheydon/solo-dev-board/issues/292), Triage action-surface usability [#492](https://github.com/markheydon/solo-dev-board/issues/492).
...
#### Hardcoding audit (v1.1 guardrail)
- [x] Audit shipped `src/` and User Guide for maintainer identity / repo assumptions; classify acceptable vs must fix. _(#423; see [`HARDCODING_AUDIT_v1.1.md`](HARDCODING_AUDIT_v1.1.md).)_

#### Daily Focus
- [x] Design `DailyFocusReport` and `BoardSnapshot` domain records
- [x] Implement `DailyFocusService` in `Application` (board state, stalled items, top-priority recommendations)
- [x] Build MudBlazor Daily Focus view: board state summary, stalled item alerts, top-3 recommended work items
- [x] Implement stalled item detection (Up Next for 3+ days; PRs In Review for 3+ days)
- [x] Write unit tests for `DailyFocusService`
- [x] Update `website/content/docs/planning.md`

#### Backlog Review
- [x] Implement `BacklogReviewService` in `Application` (cross-repo, priority-grouped, PR-aware)
- [x] Build MudBlazor Backlog Review view: groups for urgent, ready, blocked, deferred; neglected repo alerts
- [x] Implement neglected repo detection (no issue or PR activity in 14 days)
- [x] Write unit tests for `BacklogReviewService`
- [x] Update `website/content/docs/planning.md`

#### Iteration Planning
- [x] Design `IterationPlan` domain record
- [x] Implement `IterationPlanningService` in `Application` (capacity enforcement, stale resolution, milestone assignment)
- [x] Build MudBlazor Iteration Planning view: capacity indicator, stale item resolution, Up Next curation, optional milestone assignment
- [x] Write unit tests for `IterationPlanningService`
- [x] Update `website/content/docs/planning.md`

#### Repo Management
- [x] Implement excluded-repos configuration (persisted per user, applied to all cross-repo operations)
- [x] Build MudBlazor settings UI for managing excluded repositories
...
## v1.1 - Cross-Repo Planning & Refinement — remaining release work
...
- [x] Align One-Click Migration label Overwrite with the Label Manager keep-`area/*` rule (default on), including preview/apply, tests, and User Guide. _(#464, then #471)_
- [x] Relocate persistent feedback regions and remove empty Status panels. _(#473)_
...
- Independent of unmilestoned items (#293, later catalogue hygiene features, and window-focus refetch [#450](https://github.com/markheydon/solo-dev-board/issues/450)).
...
**Status:** Ready to tag — **30 closed, 0 open** on GitHub milestone `v1.2 - Planning polish, Reload & Templates` (declared 2026-08-31).

### Key Tasks

- [x] Optional board-scoped Daily Focus recommendations, default remains all included repositories. _(#403)_
- [x] Reload from GitHub on Board Rules, keeping repository (and comparison) selection. _(#449)_
- [x] Roll Reload from GitHub out to remaining GitHub-backed surfaces. _(#451, parent epic #447)_
- [x] Split hosted GitHub OAuth and API HttpClients for REST header defaults. _(#453)_
...
- [x] Unify the Triage action surface into Save and next (process) versus duplicate versus skip — usability only, not opinionated-workflow integration. _(#492)_
...
- Independent of hosted private Projects v2 (#293) and the Aspire PM settings store (#391).
- #450 remains a later child of #447, ice-boxed and unmilestoned, and is not a v1.2 close-out gate.
