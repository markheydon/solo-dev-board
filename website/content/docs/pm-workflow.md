...
If no unblocked items remain, an informational alert explains that there is nothing to recommend today. If any included repository fails to load, or the recommendation catalogue cannot be loaded, a separate error alert includes **Retry** rather than an empty list; occupancy can still show on its own while recommendations continue to load.
