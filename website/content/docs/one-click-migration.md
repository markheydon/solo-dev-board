...
- Use Label Manager when you need label-specific operations such as creating, renaming, recolouring, deleting, remapping extras onto taxonomy names, or re-synchronising labels without treating the whole repository setup as a migration.

One-Click Migration covers labels, milestones, and Projects v2 Status columns (the board-view Status field). It copies label definitions between repositories; it does not retag historical issues and pull requests. Use Label Manager **Remove labels outside taxonomy** for in-repository remap. One-Click Migration does not copy cards, views, automations, or other custom fields.
...
- You want an ongoing operational tool for label housekeeping, taxonomy enforcement, or remapping extras onto taxonomy names. Use Label Manager.
