...
/// <summary>Tests for <see cref="PlanningWorkItemCatalogueService"/>.</summary>
public sealed class PlanningWorkItemCatalogueServiceTests
{
    ...
    private readonly PlanningWorkItemCatalogueService _sut;

    public PlanningWorkItemCatalogueServiceTests()
    {
        _pmSettingsService.GetSettingsAsync().Returns(PlanningSettingsDefaults.Create());
        _sut = new PlanningWorkItemCatalogueService(
            _gitHubService,
            _pmSettingsService,
            NullLogger<PlanningWorkItemCatalogueService>.Instance);
    }
    ...
    public async Task GetCatalogueAsync_ActiveRepositoriesExist_ReturnsMappedOpenItems()
    {
        ...
        var issue = Assert.Single(result.Items, item => item.ItemType == PlanningWorkItemTypeDto.Issue);
        ...
        var pullRequest = Assert.Single(result.Items, item => item.ItemType == PlanningWorkItemTypeDto.PullRequest);
    ...
    public async Task GetCatalogueAsync_ExcludedRepositoryConfigured_OmitsExcludedRepository()
    {
        ...
        _pmSettingsService.GetSettingsAsync().Returns(new PlanningSettingsDto(
            PlanningBoardNodeId: null,
            ExcludedRepositories: ["owner/excluded"],
            Capacity: PlanningSettingsDefaults.Capacity,
            StallDays: PlanningSettingsDefaults.StallDays,
            NeglectDays: PlanningSettingsDefaults.NeglectDays));
    ...
    public async Task GetCatalogueAsync_GroupingPredicates_AreAvailableOnReturnedLabels()
    {
        ...
        Assert.True(PlanningLabelHelpers.IsUrgent(urgentReady.Labels));
        Assert.True(PlanningLabelHelpers.IsUnblocked(urgentReady.Labels));
        Assert.False(PlanningLabelHelpers.IsAwaitingTriage(urgentReady.Labels));

        var awaitingTriage = result.Items.Single(item => item.Number == 2);
        Assert.True(PlanningLabelHelpers.IsAwaitingTriage(awaitingTriage.Labels));

        var blocked = result.Items.Single(item => item.Number == 3);
        Assert.True(PlanningLabelHelpers.IsBlockedOrDeferred(blocked.Labels));
        Assert.False(PlanningLabelHelpers.IsUnblocked(blocked.Labels));
    }
