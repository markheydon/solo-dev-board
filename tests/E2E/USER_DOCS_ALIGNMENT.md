...
| [Actions Templates](../../website/content/docs/actions-templates.md) | `/actions-templates` | `actions-templates.spec.ts`, `accessibility.spec.ts` | `actions-templates/overview.png` | Tier 2 | Guide is Partially Available: browse, custom template repositories, parameterise, apply, and drift now; persisted parameter profiles ([#436](https://github.com/markheydon/solo-dev-board/issues/436)) later. CI asserts built-in template browse/filter/select, custom source field shell, and repository error state. |
