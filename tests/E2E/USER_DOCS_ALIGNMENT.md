...
| [Triage UI](../../website/content/docs/triage-ui.md) | `/triage` | `triage.spec.ts`, `accessibility.spec.ts` | `triage-ui/overview.png` | Tier 2 | Guide describes disposition-based Save and next flow, shortcuts, milestones, and project board actions in one commit; CI asserts not-started region and inline repository load failure with retry. |
| [Actions Templates](../../website/content/docs/actions-templates.md) | `/actions-templates` | `actions-templates.spec.ts`, `accessibility.spec.ts` | `actions-templates/overview.png` | Tier 2 | Guide is Partially Available: browse, custom template repositories, parameterise, apply, and drift now; persisted parameter profiles ([#436](https://github.com/markheydon/solo-dev-board/issues/436)) later. CI asserts built-in template browse/filter/select, built-in-only catalogue with empty custom source, custom source region and manual field shells (catalogue selector when repositories load), invalid custom source error with built-ins visible, and repository error state ([#499](https://github.com/markheydon/solo-dev-board/issues/499)). |
