...
| [Triage UI](../../website/content/docs/triage-ui.md) | `/triage` | `triage.spec.ts`, `accessibility.spec.ts` | `triage-ui/overview.png` | Tier 2 | Guide describes disposition-based Save and next flow, shortcuts, milestones, project board actions, and **Reload from GitHub** in session scope; CI asserts session-scope reload control, inline repository load failure with retry, and disposition controls hidden before a session starts. |
| [Actions Templates](../../website/content/docs/actions-templates.md) | `/actions-templates` | `actions-templates.spec.ts`, `accessibility.spec.ts` | `actions-templates/overview.png` | Tier 2 | Guide is Partially Available: browse, custom template repositories, parameterise, apply, drift, and **Reload from GitHub** now; persisted parameter profiles ([#436](https://github.com/markheydon/solo-dev-board/issues/436)) later. CI asserts built-in template browse/filter/select, reload control, built-in-only catalogue with empty custom source, custom source region and manual field shells (catalogue selector when repositories load), invalid custom source error with built-ins visible, and repository error state ([#499](https://github.com/markheydon/solo-dev-board/issues/499)). |
...
| Dual source picker, selector/manual sync, Load, source badge, last-used pre-fill, error leaves built-ins visible | `ActionsTemplatesTests` | `actions-templates.spec.ts` built-in-only catalogue, invalid source error shell |
