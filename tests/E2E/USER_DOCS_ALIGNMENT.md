...
| [Label Manager](../../website/content/docs/label-manager.md) | `/labels` | `labels.spec.ts`, `accessibility.spec.ts` | `label-manager/overview.png` | Tier 2 | Guide describes three tabs, taxonomy workflows, optional **Remove labels outside taxonomy** strict delete ([#380](https://github.com/markheydon/solo-dev-board/issues/380)), and nested **Keep `area/*` labels** ([#446](https://github.com/markheydon/solo-dev-board/issues/446)); CI asserts tab strip, remove-outside control, sync keep-area control, disabled actions, and no-repositories message. |
