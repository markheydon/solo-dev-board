...
| Preview lists remap rows and unused extras; keep `area/*` excludes remap sources | `LabelServiceTests` preview matrix, `LabelsTests` remap table | `labels.spec.ts` remove-outside control only (remap table needs live extras) |
| Apply retags then deletes sources; failed retags keep the source; snackbar remapped count | `LabelServiceTests`, `RecommendedTaxonomyApplySummaryHelperTests`, `LabelsTests` **Apply remap** | — |
