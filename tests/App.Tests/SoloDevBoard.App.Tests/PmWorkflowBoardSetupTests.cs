...
public sealed class PmWorkflowBoardSetupTests
{
    ...
    [Fact]
    public async Task PmWorkflowShell_WhenBoardHasCompatibilityIssues_ShowsBoardSetupTabAndSummary()
    {
        ConfigureDefaults();
        var compatibilityService = PmWorkflowTestChromeDependencies.CreateBoardCompatibilityService(_ => new PlanningBoardCompatibilityReportDto(
            "PVT_board",
            [
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            Assert.Contains("data-testid=\"pm-workflow-board-compatibility-summary\"", cut.Markup);
            Assert.Contains("Missing Todo status column", cut.Markup);
        });
    }

    [Fact]
    public async Task PmWorkflowBoardSetup_WhenIssuesExist_ShowsGroupedPanels()
    {
        ConfigureDefaults();
        var compatibilityService = PmWorkflowTestChromeDependencies.CreateBoardCompatibilityService(_ => new PlanningBoardCompatibilityReportDto(
            "PVT_board",
        ...
        ctx.Services.GetRequiredService<Microsoft.AspNetCore.Components.NavigationManager>()
            .NavigateTo("/pm-workflow/board-setup");
        var cut = ctx.RenderPmWorkflowPage<PmWorkflowBoardSetup>();

        cut.WaitForAssertion(() =>
        {
            Assert.Contains("data-testid=\"pm-workflow-board-setup-page\"", cut.Markup);
            Assert.Contains("data-testid=\"pm-workflow-board-setup-errors-panel\"", cut.Markup);
            Assert.Contains("data-testid=\"pm-workflow-board-setup-warnings-panel\"", cut.Markup);
            Assert.Contains("Missing Todo status column", cut.Markup);
    ...
    [Fact]
    public async Task PmWorkflowBoardSetup_WhenRecheckClicked_ReloadsCompatibilityFromGitHub()
    {
        ConfigureDefaults();
        ...
        cut.WaitForAssertion(() =>
        {
            Assert.Contains("data-testid=\"pm-workflow-board-setup-recheck\"", cut.Markup);
        });

        await cut.InvokeAsync(() => cut.Find("[data-testid='pm-workflow-board-setup-recheck']").Click());
