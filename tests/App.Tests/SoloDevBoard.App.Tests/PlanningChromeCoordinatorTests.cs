...
/// <summary>Unit tests for <see cref="PlanningChromeCoordinator"/> load caching and cancellation.</summary>
public sealed class PlanningChromeCoordinatorTests
{
    ...
    private readonly IPlanningProjectBoardDiscoveryService _projectBoardDiscoveryService =
        Substitute.For<IPlanningProjectBoardDiscoveryService>();
    ...
    public async Task CancelPendingLoad_WhenLoadIsInFlight_StopsFurtherRepositoryUpdates()
    {
        ...
        _pmSettingsService.GetSettingsAsync()
            .Returns(PlanningSettingsDefaults.Create());
    ...
    public async Task SaveSettingsAsync_ClearsDailyFocusRecommendationsCache()
    {
        ...
        await coordinator.SaveSettingsAsync(PlanningSettingsDefaults.Create());
    ...
    private void ConfigureSuccessfulLoad(bool delayRepositoryFetch = false)
    {
        ...
        _projectBoardDiscoveryService.GetPlanningBoardOptionsForRepositoriesAsync(
                Arg.Any<IReadOnlyList<RepositoryDto>>(),
                Arg.Any<CancellationToken>())
            .Returns(new PlanningProjectBoardDiscoveryDto([], 0, 0));
    ...
    private PlanningChromeCoordinator CreateCoordinator() =>
        new(
            _pmSettingsService,
            _repositoryService,
            _projectBoardDiscoveryService,
            _boardCompatibilityService,
            NullLogger<PlanningChromeCoordinator>.Instance);
