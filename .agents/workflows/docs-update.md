...
- When a published user-guide page or landing tile changes **materially in the UI**, recapture the matching `website/static/images/` screenshot in the same docs-update. Prefer `cd tests/E2E && DOCS_CAPTURE_ENABLED=1 dotnet test tests/E2E/SoloDevBoard.E2E.Tests --filter "Category=DocsCapture"` (or a focused `-g` filter) against a local app with a real PAT and `DocsCapture:Enabled=true`. See [plan/DOCS_STRATEGY.md](../../plan/DOCS_STRATEGY.md#screenshot-convention) and [tests/E2E/README.md](../../tests/E2E/README.md#documentation-screenshots).
...
- Updating `tests/E2E/SoloDevBoard.E2E.Tests/DocsCapture/` helpers so the capture shows the documented loaded state is in scope. That is not application feature code.
