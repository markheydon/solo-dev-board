...
Use `area/repositories` for the Repositories catalogue (`/repositories`), not `area/dashboard`. Use `area/planning` for Planning (`/planning`), not `area/dashboard`. Use `area/actions-templates` for Actions Templates (`/actions-templates`). See `plan/LABEL_STRATEGY.md` for the full `area/` guidance. `area/*` labels are for this repository's triage only; they are not exported via Label Manager's recommended catalogue ([#446](https://github.com/markheydon/solo-dev-board/issues/446)).
