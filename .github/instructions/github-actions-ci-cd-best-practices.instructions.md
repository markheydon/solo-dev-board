...
- CD uses three tiers via `.github/workflows/cd.yml` and reusable `.github/workflows/aspire-deploy.yml` (DEC-021): **developer** (`workflow_dispatch`, PAT), **staging** (push to `main`, GitHub App), **production** (push tag `v*`, GitHub App, required reviewers).
